...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (pathname.includes("tri")) {
      ...
      if (p && p.CO_POSICAO) {
        for (let i = start; i <= end; i++) {
          const idx = Object.keys(p.CO_POSICAO).find((index) => {
            const matchProva = Number(p.CO_PROVA[index]) === Number(codigo);
            const matchPos = Number(p.CO_POSICAO[index]) === i;
            ...
            if (
              ...
              return Number(p.TP_LINGUA[index]) === Number(lingua);
            }
            if (versao === "D" && p.TP_VERSAO_DIGITAL) {
              if (Number(versao) !== Number(p.TP_VERSAO_DIGITAL[index]))
                return false;
            }
            return true;
          });
    ...
    if (pathname.includes("resposta-ao-item")) {
      ...
      if (p && p.CO_POSICAO) {
        for (let num = start; num <= end; num++) {
          const idx = Object.keys(p.CO_POSICAO).find((i) => {
            ...
            if (versao === "D" && p.TP_VERSAO_DIGITAL) {
              if (Number(versao) !== Number(p.TP_VERSAO_DIGITAL[i]))
                return false;
            }
            return true;
          });
          ...
          if (idx) {
            result[num] = Number(p.CO_ITEM[idx]);
          }
        }
      }
